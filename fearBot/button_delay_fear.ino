...
//  pinMode(PLAYNEXTCLIPPIN, INPUT_PULLUP);
  ...
  int reading = digitalRead(SERVOXMITPIN);
  ...
  previousState = digitalRead(SERVOXMITPIN);

  
//  if (digitalRead(SERVOXMITPIN) == LOW ) {  // remember switches are active LOW

     // if the button is still pressed don't do this too often
//  }
...
//  pixels.show();
  ...
  if (currentMillis - previousMillis >= delayTime) {
    ...
    if (neoState == LOW) {
      for (int i = 0; i < NUMPIXELS; i++) {  // For each pixel...
        pixels.setPixelColor(i, pixels.Color(80, 80, 250));
      }
      pixels.show();
      neoState = HIGH;
    } else {
